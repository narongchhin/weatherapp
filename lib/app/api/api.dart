...
class WeatherAPI {
  ...
  static const String _weatherParams30days =
      'hourly=temperature_2m,relativehumidity_2m,apparent_temperature,precipitation,rain,weathercode,surface_pressure,visibility,evapotranspiration,windspeed_10m,winddirection_10m,windgusts_10m,cloudcover,uv_index,dewpoint_2m,precipitation_probability,shortwave_radiation'
      '&daily=et0_fao_evapotranspiration,weathercode,temperature_2m_max,temperature_2m_min,apparent_temperature_max,apparent_temperature_min,sunrise,sunset,precipitation_sum,precipitation_probability_max,windspeed_10m_max,windgusts_10m_max,uv_index_max,rain_sum,winddirection_10m_dominant'
      '&forecast_days=12&timezone=auto';

  static const String _weatherParamsForFilter =
      'daily=et0_fao_evapotranspiration,precipitation_sum,rain_sum'
      '&timezone=auto';
  ...
  String _buildWeatherUrl(double lat, double lon) {
    ...
    int ytd = date.day - 1;
    int day = date.day - 30;
    String enddate = "${date.year}-${date.month}-$ytd";

    var newDate = DateTime(date.year, date.month - 1, date.day);
    var newDate2 = DateTime(date.year, date.month, date.day - 1);

    String formattedDate = DateFormat('yyyy-MM-dd').format(newDate);
    String formattedDate2 = DateFormat('yyyy-MM-dd').format(newDate2);

    String startdate = "${newDate.year}-${newDate.month}-${newDate.day}";
    url += '&start_date=$formattedDate&end_date=$formattedDate2';
    return url;
  }
  ...
  getDateRangeDataApi(
    ...
    final String urlWeather =
        _buildWeatherUrlFilter(lat, lon, startdate, enddate);
    try {
      Response response = await dio.get(urlWeather);
      WeatherDataApi weatherData = WeatherDataApi.fromJson(response.data);
      return weatherData;
  ...
  Future<WeatherDataApi> getWeatherFiltered(
      double lat, double lon, String startdate, String enddate) async {
    final String urlWeather =
        _buildWeatherUrlFilter(lat, lon, startdate, enddate);
    try {
